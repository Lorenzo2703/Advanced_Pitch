...
def main(
    source: str,
    output: str,
    batch_size: int, #quanti elementi nella batch mettere
    shuffle_size: int,  #quanti elementi nel buffer mettere per il rimescolamento del dataset
    learning_rate: float,
    epochs: int,
    steps_per_epoch: int, #quante batch per epoca per il training
    validation_steps: int, #quante batch per epoca per la validazione
    size_evaluation_callback_datasets: int,
    datasets_to_use: List[str],
    dataset_sampling_frequency: np.ndarray,  #pesi per la creazione del dataset
    no_contours: bool,
