...
N_SAMPLES_PER_TRACK = 20 #numero di sample per il training per ogni traccia
...
def sample_datasets(
    ...
    #crea un generatore  di fpath ai tfrecords, False or True
    ...
    return tf.data.Dataset.choose_from_datasets(ds_list, choice_dataset) #prduce un dataset infinito con elementi presi dai dataset in ds_list, per questo serve introdurre numero di batch per epoch
...
def transcription_file_generator(
    ...
    ## dictionary with key as dataset name to use and value as list of paths to tfrecord files
...
def _train_file_generator(x, weights):
    ...
    while all(x.values()):
        ...
        fpath = x[np.random.choice(keys, p=weights)].pop() #prende l'ultimo elemento della lista
        yield fpath
...
#LEGA GLI AUDIO ALLE ANNOTATIONS
def transcription_dataset(file_generator, n_samples_per_track, random_seed):
    ...
    """
    ds = tf.data.Dataset.from_generator(file_generator, output_types=tf.string, output_shapes=()) #yield di fpath
    ds = tf.data.TFRecordDataset(ds)
    ds = ds.map(parse_transcription_tfexample, num_parallel_calls=tf.data.AUTOTUNE) #parse delle stringhe serializzate del tfrecord e le trasforma in tensori sparse
    ...
    ds = ds.map(
        lambda file_id, source, audio_wav, notes_indices, notes_values, onsets_indices, onsets_values, contours_indices, contours_values, notes_onsets_shape, contours_shape: (  # noqa: E501
            ...
            tf.audio.decode_wav(
                ...
                desired_samples=-1,
                name=None,
            ), #returns a tuple of audio and sample rate
    ...
    ds = ds.map(reduce_transcription_inputs) # crea una tupla con gli oggetti che servono
    ds = ds.map(get_sample_weights, num_parallel_calls=tf.data.AUTOTUNE) #serve per eliminare gli esempi con annotazioni vuote
    ds = ds.flat_map(
        lambda a, o, c, n, ow, cw, nw, m: get_transcription_chunks(
            a, o, c, n, ow, cw, nw, n_samples_per_track, random_seed
        )
    ) # estrae delle finestre casuali di lunghezza fissata e le mette in un batch di dimensione n_samples_per_track e poi le concatena
...
def parse_transcription_tfexample(
    serialized_example: tf.train.Example, #viene passato un tf.train.Example che e' un tfrecord
) -> Tuple[
    tf.Tensor,
    tf.Tensor,
    tf.Tensor,
...
def sparse2dense(values: tf.Tensor, indices: tf.Tensor, dense_shape: tf.Tensor) -> tf.Tensor:
    ...
    sp = tf.SparseTensor(indices=indices, values=values, dense_shape=dense_shape) #era tutto salvato quando ho serializzato sul tfrecords
...
def reduce_transcription_inputs(
    ...
    ) ##dovrebbe essere giusto perche' ho forzato la conversione a 44100 nella serializzazione
    return (
...
def _infer_time_size(onsets: tf.Tensor, contour: tf.Tensor, notes: tf.Tensor) -> tf.Tensor:
    """
    Potremmo non avere onset ma avere dei contour e note quindi serve capire la dimensione temporale
...
def get_sample_weights(audio, onsets, contour, notes, metadata):
    ...
    onsets = tf.cond(
        ...
        lambda: tf.zeros(
            ...
            dtype=tf.float32,
        ), #se non ci sono onset metto una matrice di zeri
        lambda: onsets,
    )
    contour = tf.cond(
    ...
    return (
        ...
        contour_weight,
        note_weight,
        metadata, # metadata e' un dizionario con file_id e src
    )


def trim_time(data, start, duration, sr): #sr = sample rate
    """
    Slice a data file
    Args:
    ...
    begin = (n_start, 0) # 0 e -1 si riferiscono all'asse delle intensita` e delle frequenze
    size = (n_duration, -1)
...
def extract_random_window(audio, onsets, contour, notes, seed):
    ...
    t_start = tf.random.uniform(
        ...
        seed=seed, #solo nel validation set voglio che sia deterministico 
    )
...
def is_not_all_silent_annotations(a, o, c, n, ow, cw, nw):
    ...
    notes_nonsilent = tf.math.reduce_mean(n) != 0 #se non ci sono note o contour non ha senso fare training e non ci sono onset
