...
class SlakhFilterInvalidTracks(beam.DoFn):
    ...
    def setup(self):
        ...
        self.slakh_remote = mirdata.initialize("slakh", version='baby', data_home=os.path.join(self.source, "slakh"))
    ...
    def process(self, element: Tuple[str, str]):
        ...
        with tempfile.TemporaryDirectory() as local_tmp_dir:
            slakh_local = mirdata.initialize("slakh", version='baby', data_home=local_tmp_dir)
...
def create_input_data(
    train_percent: float, validation_percent: float, seed = None
) -> List[Tuple[str, str]]:
    ...
    slakh = mirdata.initialize("slakh", version="baby")
    slakh.download()

    return [(track_id, determine_split()) for track_id in slakh.track_ids]
